...
1. This report investigates data from the 1948 [Framingham Heart Study](https://www.framinghamheartstudy.org).

- The longitudinal study includes 4,133 participants with 13 factors total over 10 years. 

- Using this data, I will investigate the risk factors for cardiovascular disease.


2. This report and my article <<[Introduction to Logit](https://github.com/mcc-us/intro-2-logit/blob/main/intro-2-logit.pdf)>> display my understanding of logistic regression and R. 


3. Seven (7) of the 13 factors are have high correlation leading to cardiovascular disease. The odds related to each factor were calculated from the study.
...
print(paste("The minimum age is", min_age))
print(paste("The maximum age is", max_age))
```
...
ggplot(males, aes(x = age)) +
  geom_histogram(bins = 20, color="black", fill = "#D4F5FF") +
...
ggplot(females, aes(x = age)) +
  geom_histogram(bins = 20, color = "black", fill = "#FFD9EC") +
...
```{r echo=FALSE, fig.width=3, message=FALSE, warning=FALSE}
# Create a histogram of ages for males
ggplot(males, aes(x = age)) +
  geom_histogram(fill = "#FF99CC") +
  labs(title = "Age Distribution of Males",
