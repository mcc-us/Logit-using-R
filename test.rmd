---
title: "Determining Cardiovascular Disease Odds Given 7 Risk Factors"
subtitle: "Using Framingham Heart Study Data"
author: "By Matthew Curcio"
output: pdf_document
---

geometry: "left=1.25cm, right=1.25cm, top=1.25cm, bottom=2.5cm"
    
## 1. Executive Summary

This report investigates data from the 1948 Framingham Heart Study.^[https://www.framinghamheartstudy.org] This longitudinal study includes 4,133 participants with 13 factors total over 10 years. Using this data, I investigate the risk factors for cardiovascular disease (CVD).

1. This report and my article *Introduction to Logistic Regression*^[https://github.com/mcc-us/intro-2-logit/blob/main/intro-2-logit.pdf] provide excellent information for understanding of logistic regression and R.

2. Seven (7) factors have a strong correlation that leads to cardiovascular disease. The odds related to each factor are calculated from the study.
...
| 2   | Hi Systolic Blood Pressure             | 7.8:1 |
| 3   | Hi Glucose Levels                      | 2.5:1 |
...
Data can be found at Kaggle.^[https://www.kaggle.com/datasets/captainozlem/framingham-chd-preprocessed-data]
...
#title('Age Of Participants', cex.main=2)

boxplot(arr, ylim=c(0,70), col = "#EEEEEC")
#title('Cigarettes Per Day', cex.main=2)

boxplot(arr, ylim=c(0,70), col = "blue")
#title('Body Mass Index', cex.main=2)
```
...
title('Systolic BP')
boxplot(df$diaBP, ylim=c(0,150), col = "#EEEEEC")
title('Diastolic BP')
boxplot(df$heartRate, ylim=c(0,150), col = "blue")
title('Heart Rate')
```

#### Total Cholestrol, Glucose Concentrations
```{r}
#par(mfrow = c(1, 2))
boxplot(df$totChol, ylim=c(0,700), col = "#F97575")
title('Total Cholestrol, mg/dL')
boxplot(df$glucose, ylim=c(0,700), col = "#FCE94F")
title('Glucose, mg/dL')
```
...
#plot two histograms in same graph
hist(males, col='red')
hist(females, col='green', add=TRUE)
#add legend
legend('topright', c('x1 variable', 'x2 variable'), fill=c('red', 'green'))
```
...
plot_multi_histogram(df, 'age', 'male')
```
